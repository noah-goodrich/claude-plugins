...
- **ai-scoring**: Run the scoring pass on each variant and present the score and flags with the variant. Do not revise to clear the threshold before showing it.
...
Before presenting any variant, run **both** scoring passes and report both scores with the variant:

1. **AI Detection Score** (from `ai-scoring` skill): report the score and flag specific tells with line citations.
   Present it whatever the number is; do not revise in a loop to clear 75. That rubric flags 5 of Noah's 10
   published articles as machine-written, so treat a low score as a prompt to look rather than a verdict.
2. **LinkedIn Cringe Score** (rubric below): Must be 75+. Flag anything that reads performative, preachy, or like slop.
   This one is a locally-defined rubric, not the miscalibrated one, and it stays a threshold.
...
6. Run ai-scoring (report the score, do not gate on it) and cringe score (75+) before presenting
